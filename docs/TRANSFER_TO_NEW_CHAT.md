...
- `0.1.6` — панель запускается, адрес и порт панели задаются через env Portainer.

Текущий релиз в работе:
- `0.1.7`
- переработка UI под левое меню
- разделение dashboard и функциональных экранов
- отдельные разделы Главная / Серверы / Группы / Проверки / Оповещения / Дальше
