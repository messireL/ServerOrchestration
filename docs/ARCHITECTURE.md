...
- status-table использует компактные плашки и ограничение ширины ячеек, чтобы длинные URL/ошибки не ломали layout;
...
## Актуальное уточнение v0.1.53
- version propagation должна быть согласована между `VERSION`, backend runtime, Docker build arg и локальным compose build-контуром;
- служебные каталоги тестов/кэша не являются частью release-пакета и исключаются из дистрибутива;
- packaging hygiene считается частью архитектурной дисциплины проекта, а не «мелочью на потом».


## Актуальное уточнение v0.1.54
- Вкладка `Проверки` перестроена как operational-экран: быстрые действия, статусы, планировщик и история больше не смешаны в одну перегруженную простыню.
- Локальные фильтры/поиск в `Проверки` не зависят от фильтров inventory, чтобы не было путаницы между разделами.
- История прогонов теперь имеет клиентские фильтры по источнику (`scheduler`/ручные) и лимит вывода, чтобы быстрее читать свежие события.
